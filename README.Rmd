...
For R >= 4.4.0, the [quickr](https://CRAN.R-project.org/package=quickr) package can be used to accelerate `statistic`. However, this results in repeated crossings of the R-Fortran boundary and makes pre-calculation of constants impossible.

```{asciicast, define_quickr}
t_quickr <- define_pmt(
